...
from models.networks.laa_docl_net import Unet_dsbn_cont
...
class DCLW(nn.Module):
    ...
    def weight_fn(self, z1, z2):
        similarities = self.cosine_similarity(z1, z2)
        return 2 - z1.size(0) * F.softmax(similarities / self.sigma, dim=0).squeeze()

    def forward(self, z1, z2,is_pos=True):
        cross_view_similarity = self.cosine_similarity(z1, z2)
        positive_loss = -torch.diag(cross_view_similarity) / self.temperature

        weight = self.weight_fn(z1, z2)
        positive_loss = positive_loss * weight

        neg_similarity = torch.cat((self.cosine_similarity(z1, z1), cross_view_similarity), dim=1) / self.temperature
...
def _val_on_the_fly(model:nn.Module, loader_val:list, writer, iter_num):
    ...
    for num, batch_val in enumerate(loader_val):
        ...
        with torch.no_grad():
            pred_val, _ = model(img_val, domain_label=1,is_td=False)
        pred_val = F.softmax(pred_val, dim=1)
        soft_gt = losses.get_soft_label(gt_val, net_params['num_classes_t'])
        loss_bce_val += bce_criterion(pred_val, soft_gt.permute(0, 3, 1, 2).long())
        pred_mask = torch.argmax(pred_val, dim=1, keepdim=True)
        pred_softmask = losses.get_soft_label(pred_mask, net_params['num_classes_t'])
        dice_val += losses.val_dice_class(pred_softmask[..., 1:], soft_gt[..., 1:], net_params['num_classes_t']-1)
...
def train(model:nn.Module, ema_model:nn.Module, loader_train_s:list, 
    ...
    focal_criterion = losses.FocalLoss()
    similar_criterion = dcl.DCL(temperature=0.5)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for k, batch_s in enumerate(loader_train_s):
            ...
            model.train()
            ema_model.train()
            predouts, high_r_s_sb  = model(image_s, domain_label=0,is_td=False)
            predoutt, high_r_t_tb  = model(image_t, domain_label=1,is_td=False)
            _, high_r_s_tb = model(image_s, domain_label=1,is_td=False)
            _, high_r_t_sb = model(image_t, domain_label=0,is_td=False)

            with torch.no_grad():
                ema_output, _ = ema_model(ema_inputs_t, domain_label=1,is_td=False)

            # segmentation loss
            loss_bce_s = focal_criterion(predouts, label_s.squeeze().long())
            ...
            loss_bce_t = focal_criterion(predoutt[:train_params['labeled_bs']], label_t[:train_params['labeled_bs']].squeeze(dim=1).long())
            predout_t = F.softmax(predoutt, dim=1)
            soft_label_t = losses.get_soft_label(label_t[:train_params['labeled_bs']], net_params['num_classes_t'])
            loss_dice_t = dice_criterion(predout_t.permute(0, 2, 3, 1)[:train_params['labeled_bs']][..., 1:], soft_label_t[..., 1:], net_params['num_classes_t']-1)
            loss_t = 0.5 * (loss_bce_t + loss_dice_t)

            consistency_loss = 0
            consistency_weight = get_current_consistency_weight(iter_num//len(loader_train_s), max_epoch)
            consistency_dist = consistency_criterion(predout_t[train_params['labeled_bs']:], ema_output)    #(batch, 4, 256, 256)
            consistency_dist = torch.mean(consistency_dist)
            consistency_loss = consistency_dist * consistency_weight

            # contrastive loss
            pos_s2t_similar = similar_criterion(high_r_s_sb, high_r_t_tb,True) / train_params['temp_fac']
            den_s2t1_similar = similar_criterion(high_r_s_sb, high_r_s_tb,False) / train_params['temp_fac']
            den_s2t2_similar = similar_criterion(high_r_s_sb, high_r_t_sb,False) / train_params['temp_fac']
            ...
            pos_t2s_similar = similar_criterion(high_r_t_tb, high_r_s_sb,True) / train_params['temp_fac']
            den_t2s1_similar = similar_criterion(high_r_t_tb, high_r_t_sb,False) / train_params['temp_fac']
            den_t2s2_similar = similar_criterion(high_r_t_tb, high_r_s_tb,False) / train_params['temp_fac']
            ...
            contrast_loss_inter = (pos_s2t_similar +den_s2t1_similar + den_s2t2_similar + pos_t2s_similar + den_t2s1_similar + den_t2s2_similar)/6
            contrast_loss = 0.1*torch.mean(contrast_loss_inter)

            # total loss
            # loss = 0.3*loss_s + loss_t + consistency_loss + contrast_loss
            loss = 0.5*(0.8*loss_s+loss_t) + consistency_loss + 0.5*contrast_loss
            ...
            if iter_num % len(loader_train_s) == 0:
                ...
                # logging.info('Validation --> loss_bce: %.4f; loss_dice: %.4f; mean_dice: %.4f (RNFL DICE: %.4f; GCIPL DICE: %.4f; choroid DICE: %.4f)' %
                            # (loss_bce_val.item(), loss_dice_val.item(), torch.mean(dice_val_class).item(), dice_val_class[0].item(), dice_val_class[1].item(),dice_val_class[2].item()))
                logging.info('Validation --> loss_bce: %.4f; loss_dice: %.4f; mean_dice: %.4f (RNFL DICE: %.4f; GCIPL DICE: %.4f;)' %
                ...
                if torch.mean(dice_val_class) > torch.mean(best_val_dice):
                    ...
                    # logging.info('********** Best model (dice: %.4f; RNFL DICE: %.4f; GCIPL DICE: %.4f; choroid DICE: %.4f) is updated at step %d.' %
                                # (torch.mean(dice_val_class).item(), dice_val_class[0].item(), dice_val_class[1].item(), dice_val_class[2].item(), iter_num))
                    logging.info('********** Best model (dice: %.4f; RNFL DICE: %.4f; GCIPL DICE: %.4f;) is updated at step %d.' %
                                (torch.mean(dice_val_class).item(), dice_val_class[0].item(), dice_val_class[1].item(), iter_num))
                else:
                    # logging.info('********** Best model (dice: %.4f; RNFL DICE: %.4f; GCIPL DICE: %.4f; choroid DICE: %.4f) was at step %d, current dice: %.4f.' %
                                # (torch.mean(best_val_dice).item(), best_val_dice[0].item(), best_val_dice[1].item(), best_val_dice[2].item(), best_val_step, torch.mean(dice_val_class).item()))
                    print(best_val_dice)
                    print(best_val_step)
                    print(dice_val_class)
                    logging.info('********** Best model (dice: %.4f; RNFL DICE: %.4f; GCIPL DICE: %.4f; was at step %d, current dice: %.4f.' %
...
if __name__ == '__main__':
    ...
    dataset_valid_t = CmrsegDataset(data_list_dir=data_params['target_data_list_dir'], data_dir=data_params['target_data_dir'],
                                 train_type='valid', image_type='image', transform=transforms.Compose([ToTensor()]))
    ...
    # print(dataset_train_t.__getitem__(50)['label'])
